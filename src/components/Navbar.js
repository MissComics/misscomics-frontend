...
function BurgerIcon({ isOpen }) {
  ...
  return (
    <div className="relative w-10 h-10 flex items-center justify-center select-none">
      ...
      {/* ── Intact burger — visible when closed ── */}
      <div
        className="absolute inset-0 flex items-center justify-center"
        style={{
          transition: "all 400ms cubic-bezier(0.34, 1.56, 0.64, 1)",
          opacity: isOpen ? 0 : 1,
          transform: isOpen ? "scale(0.6) rotate(-10deg)" : "scale(1) rotate(0deg)",
          pointerEvents: isOpen ? "none" : "auto"
        }}
      >
        <Image
          src="/icons/burger-complete.svg"
          alt="menu"
          width={36}
          height={36}
          priority
      ...
      {/* ── Exploded layers — visible when open ── */}
      <div
      ...
      <nav className="hidden md:flex relative items-center justify-between
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            {pinnedBurgerItem ? (
              <Link
                href={pinnedBurgerItem.href}
                title={`${pinnedBurgerItem.label} (pinned)`}
                className="w-9 h-9 flex items-center justify-center rounded-xl
                           bg-gray-900 hover:bg-gray-800 transition text-gray-300
                           text-xs font-semibold px-2">
                {pinnedBurgerItem.label}
              </Link>
            ) : (
          ...
          {/* Community */}
          <Link href="/community"
            className="text-gray-500 hover:text-white transition-colors p-2.5
                       rounded-xl hover:bg-gray-900 flex items-center justify-center">
          ...
          <div className="relative" ref={burgerRef}>
            ...
            {burgerOpen && (
              <div className="absolute right-0 mt-3 w-56 bg-gray-950
                ...
                {subPanelOpen && (
                  <div className="bg-gray-900 rounded-xl p-1.5 flex flex-col gap-0.5 mt-1">
          ...
          <div className="relative" ref={settingsRef}>
            <button
              ...
              className="text-gray-500 hover:text-white transition p-2.5
                         rounded-xl hover:bg-gray-900 flex items-center justify-center"
              aria-label="Settings">
              <Settings className={`w-5 h-5 stroke-[1.8] transition-transform 
                duration-500 ${settingsOpen ? 'rotate-90 text-purple-400' : ''}`} />
            </button>
            ...
            {settingsOpen && (
              <div className="absolute right-0 mt-3 w-56 bg-gray-950
                ...
                <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl
                  ...
                  <span className="flex-1 text-xs">UI Experience</span>
                  <div className="flex bg-gray-800 rounded-lg p-0.5 gap-0.5">
                    <button
                      onClick={() => setDarkMode(false)}
                      className={`text-[10px] px-1.5 py-0.5 rounded-md font-semibold transition
                ...
                <button className="flex items-center gap-3 px-3 py-2.5 rounded-xl
                                   text-gray-400 hover:bg-red-950/40 hover:text-red-400
                                   text-sm transition w-full text-left">
                  Delete My Account
                </button>
              </div>
